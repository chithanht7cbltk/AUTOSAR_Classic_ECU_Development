...
void CanIf_RxIndication(Can_IdType canId, const PduInfoType* PduInfoPtr)
{
    ...
    /* Routing theo CAN ID: chỉ gửi Diagnostic frames cho CanTp */
    if (canId == 0x7DF || canId == 0x7E8) {
        CanTp_RxIndication(0, PduInfoPtr);
    } else {
        /* COM frames (Engine 0x180, Brake 0x280, Body 0x380...) */
        /* Trong thực tế: PduR_CanIfRxIndication() */
        /* Demo: bỏ qua vì RX node không xử lý COM signals */
    }
}
