...
void CanTp_RxIndication(PduIdType CanTpRxPduId, const PduInfoType* CanTpRxPduPtr) {
    ...
    if (pciType == 0x01) {
        ...
        /* Copy 6 bytes dữ liệu từ FF vào buffer */
        memcpy(RxState.RxBuffer, &CanTpRxPduPtr->SduDataPtr[2], 6);
    ...
    /* ===== Xử lý Consecutive Frame (CF) ===== */
        ...
        /* ===== BƯỚC 2: Copy dữ liệu từ CF vào buffer ===== */
        memcpy(&RxState.RxBuffer[RxState.ReceivedLength],
               &CanTpRxPduPtr->SduDataPtr[1],  /* Skip PCI byte */
               len);
